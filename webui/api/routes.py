...
def _handle_chat_sync(handler, body):
    ...
    try:
        ...
        with CHAT_LOCK:
            ...
            # Fallback: read api_key directly from config.yaml model.api_key
            # when resolve_runtime_provider couldn't find it (e.g. custom provider)
            if not _api_key:
                try:
                    _model_cfg = cfg.get("model", {})
                    if isinstance(_model_cfg, dict):
                        _cfg_key = (_model_cfg.get("api_key") or "").strip()
                        if _cfg_key:
                            _api_key = _cfg_key
                except Exception:
                    pass
...
def _handle_session_compress(handler, body):
    ...
    try:
        ...
        if not resolved_api_key:
            try:
                _model_cfg = _cfg.cfg.get("model", {})
                if isinstance(_model_cfg, dict):
                    _cfg_key = (_model_cfg.get("api_key") or "").strip()
                    if _cfg_key:
                        resolved_api_key = _cfg_key
            except Exception:
                pass
