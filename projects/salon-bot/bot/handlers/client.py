"""
Salon Bot — Client (Booking Flow 2026)
Backend Architect: clean state machine, Mini App ready.
"""
...
def _fmt_booking(b: dict) -> str:
    return (
        ...
        f"👩‍🎨 {b.get('master_name', '?')}\n"
        f"📅 {_fmt_date(b['booking_date'])} в {b['time_slot']}"
    )
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    from bot.lumina import get_greeting, predict_next_visit
    greeting = get_greeting(client.get("name", ""))

    # Check if predictive recommendation applies
    history = await db.get_client_bookings(client["id"])
    completed = [b for b in history if b["status"] in ("completed", "confirmed")]
    prediction = predict_next_visit(completed)
    ...
    await msg.answer(welcome, reply_markup=kb.main_menu_kb(), parse_mode="HTML")
...
async def show_contacts(cb: CallbackQuery):
    ...
    if config.SALON_TG:
        text += f"📱 Telegram: {config.SALON_TG}\n"
    if config.SALON_INSTAGRAM:
        text += f"📸 Instagram: {config.SALON_INSTAGRAM}\n"

    await cb.message.edit_text(text, reply_markup=kb.back_to_menu_kb(), parse_mode="HTML")
...
async def leave_review(cb: CallbackQuery):
    ...
    if not completed:
        await cb.message.edit_text(
            "У вас пока нет завершённых визитов, чтобы оставить отзыв.",
            reply_markup=kb.main_menu_kb(),
        )
        return
...
async def book_start(cb: CallbackQuery, state: FSMContext):
    ...
    # Try rich table first, fallback to text
    try:
        ...
        if available:
            ...
            if rich:
                ...
                await cb.message.answer(
                    rich_message=rich,
                    reply_markup=kb.services_kb(services),
                )
                return
    except Exception:
        pass
...
async def choose_service(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f"<b>{service.get('emoji','💇')} {service['name']}</b> — {service['price']}₽\n\n"
        f"👩‍🎨 Выберите мастера:",
...
async def choose_master(cb: CallbackQuery, state: FSMContext):
    ...
    rating_str = f"⭐ {rating} ({count} отзывов)" if count > 0 else "Нет отзывов"

    await state.update_data(master_id=master_id, master_name=master["name"])
    await state.set_state(BookingState.choosing_date)
    await cb.message.edit_text(
        f"👩‍🎨 <b>{master['name']}</b> — {rating_str}\n\n📅 Выберите дату:",
...
async def choose_date(cb: CallbackQuery, state: FSMContext):
    ...
    free = await db.get_available_slots(master_id, date_iso, duration)

    if not free:
        await cb.message.edit_text(
            f"😔 На {_fmt_date(date_iso)} нет свободных слотов.\nВыберите другую дату:",
            reply_markup=kb.dates_kb(),
        )
        return
...
async def choose_time(cb: CallbackQuery, state: FSMContext):
    parts = cb.data.split("_")  # time_DATE_TIME
    date_iso = parts[1]
    time_slot = parts[2]

    data = await state.get_data()

    from bot.lumina import smart_booking_reason
    ai_reason = smart_booking_reason(
        data.get("master_name", ""), data.get("service_name", ""), time_slot
    )
    ...
    await cb.message.edit_text(
        ...
        f"{data.get('service_emoji', '💇')} {data['service_name']} — {data['service_price']}₽\n"
        f"👩‍🎨 {data['master_name']}\n"
        f"📅 {_fmt_date(date_iso)}\n"
        f"⏰ {time_slot}\n\n"
        f"<i>{ai_reason}</i>",
...
async def confirm_booking(cb: CallbackQuery, state: FSMContext):
    parts = cb.data.split("_")  # confirm_master_service_date_time
    ...
    booking_id = await db.create_booking(
        ...
        data.get("service_duration", 60), data.get("service_price", 0),
    )
    ...
    await cb.message.edit_text(
        ...
        f"{data.get('service_emoji', '💇')} {data.get('service_name', '?')}\n"
        f"👩‍🎨 {data.get('master_name', '?')}\n"
        f"📅 {_fmt_date(date_iso)} в {time_slot}\n\n"
        f"Ждём вас! 💖",
...
async def my_bookings(cb: CallbackQuery):
    ...
    for b in bookings[:10]:
        ...
        text += (
            f"{icon} {_fmt_date(b['booking_date'])} в {b['time_slot']} — "
            f"{b.get('service_name', '?')} ({b.get('master_name', '?')})\n"
        )
    ...
    if confirmed:
        b = confirmed[0]
        buttons.append(
            [InlineKeyboardButton(text="❌ Отменить последнюю", callback_data=f"del_{b['id']}")]
        )
...
async def delete_booking(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        f"❌ Запись отменена:\n\n"
        f"{_fmt_date(booking['booking_date'])} в {booking['time_slot']}\n"
...
async def process_rating(cb: CallbackQuery):
...
# ── Photo Analysis (Lumina AI) ─────────────────────────────
@router.message(F.photo)
async def analyze_photo(msg: Message):
    """User sends a photo — Lumina AI suggests services."""
    from bot.lumina import suggest_from_photo
    suggestion = suggest_from_photo()

    await msg.answer(
        f"✨ <b>Lumina AI</b> проанализировала твоё фото:\n\n"
        f"{suggestion}",
        parse_mode="HTML",
        reply_markup=kb.main_menu_kb(),
    )
