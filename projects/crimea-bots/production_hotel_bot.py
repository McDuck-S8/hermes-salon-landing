...
# SOCKS5 proxy for Russia (api.telegram.org blocked)
PROXY_URL = os.environ.get("TELEGRAM_PROXY", "socks5://2yXmZbYi:rwmKjZNa@dc01.steelproxy.com:3072")
try:
    from aiohttp_socks import ProxyConnector
    _has_proxy = True
    logging.info(f"Proxy support loaded")
except ImportError:
    _has_proxy = False
    logging.warning("aiohttp-socks not installed, connecting directly")
...
# ============ BOT ============
bot = None  # Created in main() with proxy support
...
async def main():
    ...
    # Create bot with SOCKS5 proxy support
    if _has_proxy:
        from aiogram.client.session.aiohttp import AiohttpSession
        session = AiohttpSession(proxy=PROXY_URL)
        bot = Bot(token=BOT_TOKEN, session=session)
        proxy_host = PROXY_URL.split('@')[1] if '@' in PROXY_URL else PROXY_URL
        logging.info(f"Bot {HOTEL_NAME} started with proxy: {proxy_host}")
    else:
        bot = Bot(token=BOT_TOKEN)
        logging.info(f"Bot {HOTEL_NAME} started (direct connection)")
