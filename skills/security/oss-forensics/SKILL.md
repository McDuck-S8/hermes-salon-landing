...
## Quick Local Code Audit (not a full investigation)
When a user asks to check code for malicious patterns BEFORE installing/running (e.g. "проверь на зловредный инжект"), use the quick scan pattern in [local-code-audit.md](./references/local-code-audit.md). This is a regex-based static scan, not a full forensic investigation. Use it as a pre-install safety check.
