...
#### Pitfall: recall.sh path mismatch

`.lavra/memory/recall.sh` internally looks for `.beads/memory/knowledge.jsonl` (via `MEMORY_DIR="${CLAUDE_PROJECT_DIR:-.}/.beads/memory"`), but the actual knowledge store lives at `.lavra/memory/knowledge.jsonl`. If recall.sh says "No knowledge base found", create a symlink:

```bash
mkdir -p "$PROJECT_ROOT/.beads/memory"
ln -sf "$PROJECT_ROOT/.lavra/memory/knowledge.jsonl" "$PROJECT_ROOT/.beads/memory/knowledge.jsonl"
```

Alternatively, use `knowledge-db.sh` directly for FTS5 search:

```bash
source "$PROJECT_ROOT/.lavra/memory/knowledge-db.sh"
kb_ensure_db "$PROJECT_ROOT/.lavra/memory/knowledge.db"
kb_sync "$PROJECT_ROOT/.lavra/memory/knowledge.db" "$PROJECT_ROOT/.lavra/memory"
kb_search "$PROJECT_ROOT/.lavra/memory/knowledge.db" "search terms" 10
```
