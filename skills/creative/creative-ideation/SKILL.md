...
- , video, diff, plan
Summary: # ASCII Art Skill Multiple tools for different ASCII art needs. All tools are local CLI programs or free REST APIs — no API keys required. ## Tool 1: Text Banners (pyfigle
- Integration (twozero MCP) ## CRITICAL RULES 1. **NEVER guess parameter names.** Call `td_get_par_info` for the op type FIRST. Your training data is wrong for TD 2025.32. 2. **If `tdAtt
- hybrid creative outputs. The agent coordinates multiple generative models to remix source materials: extracting visual styles from reference images, applying them to new compositions, synchronizing au
- For creative tasks, an AI agent should first clarify the core objective and desired emotional tone. It can then generate multiple diverse concepts or drafts before refining the most promising directio
- entive Knowledge Cube Entries from Verified Fixes Patterns.
