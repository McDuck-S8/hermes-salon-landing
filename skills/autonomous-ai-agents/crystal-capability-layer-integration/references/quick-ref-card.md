# Crystal Capability Layer — Quick Reference Card
...
hermes skills install omh-deep-research omh-ralplan omh-ralplan-driver omh-deep-interview omh-ralph omh-ralph-driver omh-ralph-task omh-autopilot omh-triage omh-triage-driver
hermes skills install agent-reach

# Verify
hermes skills list | grep -E "omh-|agent-reach"
```
...
# Search
videos = crystal_youtube_search("faceless YouTube automation 2024", max_results=5)
results = crystal_web_search("CPA arbitrage case study 2024", max_results=10)
...
entries = crystal_rss_fetch("https://feeds.feedburner.com/venturebeat/SZYF")
```
...
result = pipeline.scan_and_research("Telegram CPA funnel 2024", auto_plan=True)
pipeline.save_intel("Telegram CPA funnel 2024", result)

# Step by step
omh = OMHIntegration()
research = omh.run_deep_research("topic")
plan = omh.run_ralplan("requirements...")
execution = omh.run_ralph(plan)
autopilot = omh.run_autopilot(domain="...", goal="...")
```

## Crystal Full Cycle (with new phases)

```python
from scripts.crystal.core import CrystalEngine

e = CrystalEngine()
artifacts = e.run_full_cycle(quick=False)  # Includes Faza 5b + 6b
```

## Intelligence Scan (Cron-friendly)
...
# Returns list[IntelItem] ready for Knowledge Cube upsert
```

## Chain Heartbeat Commands

```bash
python scripts/syscheck.py              # Mandatory session entry
python scripts/syscheck.py --quiet      # Silent, exit code only
python scripts/system_heartbeat_fixer.py # Beats all alive modules + events
python scripts/chain_heartbeat.py status # Full status
```

## Knowledge Cube Upsert Pattern

```python
from scripts.kc_rag import upsert

upsert(
    content="...method/tool/workflow...",
    tags="faceless-youtube,ai-automation,case-study,crimea-ok",
...
| File | Purpose |
|------|---------|
| `scripts/crystal/omh_integration.py` | Main integration layer |
| `scripts/crystal/intelligence.py` | IntelScanner with Agent Reach |
| `scripts/crystal/core.py` | CrystalEngine with new phases |
| `scripts/chain_heartbeat.py` | OMH + Agent Reach pipelines |
| `scripts/kc_rag.py` | Knowledge upsert |
| `scripts/generate_portfolio.py` | Auto-regenerates docs/index.html |
