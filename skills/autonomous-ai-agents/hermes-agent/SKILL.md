...
- `references/gateway-config-proxy-debugging-2026-06-27.md` — Telegram gateway proxy configuration and debugging (SOCKS5/HTTP, timeouts, keepalive, lock files, config.yaml vs .env)
- `references/gateway-locks.md` — Gateway lock file mechanics, stale lock detection, Telegram token conflicts
- `references/portable-migration.md` — Moving Hermes between machines, HERMES_HOME resolution, profile migration
- `references/git-update-with-local-mods-2026-06-29.md` — **Conflict resolution pattern for updating hermes-agent with local modifications** (stash → ff-only → pop → manual resolve)
...
### Git-Based Update (running from source)

When hermes-agent was installed via `git clone` and is behind upstream:

**Before updating — always analyze first:**
```bash
# 1. Check what changed
cd /path/to/hermes-agent
git fetch origin
git log --oneline HEAD..origin/main   # preview total commit count
git log --oneline HEAD..origin/main --stat -- ':!package-lock.json' ':!yarn.lock' | head -60

# 2. Check your local mods won't conflict
git status --short                    # see local changes
git diff HEAD..origin/main -- path/to/modified-file.py  # check upstream touched YOUR files

# 3. Categorize upstream changes
git log --oneline HEAD..origin/main --grep="CVE\|security\|vuln"     # security
git log --oneline HEAD..origin/main --grep="fix" -- agent/ gateway/ tools/  # fixes
git log --oneline HEAD..origin/main --grep="feat" -- agent/ hermes_cli/     # features
```

**Safe update with local modifications:**
```bash
git stash -u -m "pre-update-$(date +%Y-%m-%d)"    # save local mods
git pull --ff-only origin main                      # fast-forward only (no merge commits)
git stash pop                                       # restore local mods
uv sync                                             # update dependencies
```

**Conflict resolution (when upstream touched YOUR modified files):**
```bash
# 1. Stash local changes
git stash push -m "local-config-changes" -- path/to/modified-file.py

# 2. Fast-forward update
git pull --ff-only origin main

# 3. Restore stashed changes (may have conflicts)
git stash pop

# 4. Resolve conflicts manually
#    - Open conflicted file, keep BOTH upstream improvements AND local config
...
```bash
# Core imports
python -c "import agent.conversation_loop, agent.tool_executor, tools.terminal_tool, tools.file_tools; print('OK')"

# Scripts
python -c "import sys; sys.path.insert(0, 'scripts'); from auto_recall import recall_for_session; from hermes_hooks import get_hooks; print('OK')"

# CLI
python hermes_cli/main.py --help | head -5

# Update version cache
echo '{"behind": 0, "ver": "'$(git describe --tags --always)'"}' > ../.update_check
```

**Pitfalls:**
- `hermes update` CLI auto-stashes and pulls — but does not analyze first. User preference: always analyze before applying large updates
- If `pyproject.toml` changed upstream, `uv sync` is mandatory
- `.update_check` is a cached file — manual `git pull` doesn't update it
- 318+ commit jumps may contain dependency changes, DB migrations, or breaking config changes — always check `pyproject.toml` diff
- **Windows: `hermes update` fails with "Another hermes.exe is running"** — use `--force` to override. The running hermes.exe is THIS session's process; force-updating is safe because the new exe is written to disk and scheduled for replacement on reboot
- **Windows: hermes.exe locked after update** — `uv sync` may fail to replace `venv/Scripts/hermes.exe` because it's locked by the running process. The update still succeeds for everything else (deps, code). The exe replacement happens on next reboot. Don't panic.
- **`--allow-unrelated-histories` trap** — if local repo has custom files that never existed upstream, merging with `--allow-unrelated-histories` marks them as "deleted". Use `hermes update` or stash/pull/pop instead of raw merge
...
- **All cron jobs have null last_run_at** = gateway is dead. Fix: `hermes gateway run -v`. The cron ticker is a daemon thread inside the gateway process — no gateway = no cron. Verify with `netstat -ano | findstr "9003"`.
- **Zombie gateway** = port open but PID missing from tasklist. Fix: `taskkill /PID <pid> /F` then restart gateway.
- **"Telegram bot token already in use (PID XXXX)"** = stale scoped lock file from a previous gateway (or different HERMES_HOME). The gateway writes token-scoped lock files at `~/.local/state/hermes/gateway-locks/telegram-bot-token-<hash>.lock`. If the owning process died or the install was moved, these become stale. Fix: delete the lock files and restart. See `references/gateway-locks.md` for details and the detection logic.
- **"Telegram bot token already in use" but no stale lock** = another process (e.g. salon bot, another gateway instance) is using the same bot token in polling mode. Telegram only allows ONE polling connection per bot token. Either use a separate bot token, or run one as webhook instead of polling.
- **Telegram network errors (RemoteProtocolError, ConnectError)** = unstable network to api.telegram.org. Gateway auto-reconnects with exponential backoff. If behind a VPN/proxy, check if the proxy is healthy first. On some networks (e.g. Russia), direct access works but is flaky — proxy fallback may help. Use `hermes config set telegram.proxy_url ""` to disable proxy or set a working one.
- **SOCKS5 proxy intermittently dies → HTTP proxy fallback** — v2rayN runs TWO xray processes: SOCKS5 (port 10806) and HTTP (port 10809). When SOCKS5 fails, test HTTP: `curl -x http://127.0.0.1:10809 -s --connect-timeout 8 https://api.telegram.org`. If 302, switch: `hermes config set telegram.proxy_url "http://127.0.0.1:10809"`. HTTP proxy is more stable than SOCKS5 on Windows v2rayN.
- **`keepalive_expiry=0` for proxy connections** — When using SOCKS5 proxy, stale connections cause "Server disconnected" errors. Fix: add `keepalive_expiry=0` to httpx Limits in adapter.py proxy path. Forces fresh connection per request.
- **CRITICAL: DUAL CONFIG FILES — gateway reads `~/.hermes/config.yaml`, NOT project root** — There are TWO `config.yaml` files: `~/.hermes/config.yaml` (gateway reads this, `gateway/config.py` line 811) and `D:/Portable_Soft/hermes/config.yaml` (project root — gateway IGNORES this). **Symptom:** Agent edits project-root config.yaml with correct proxy_url, gateway still connects directly. **Fix:** Always edit `~/.hermes/config.yaml` or use `hermes config set telegram.proxy_url "..."`. **Verification:** `grep proxy_url ~/.hermes/config.yaml` shows the setting.
- **CRITICAL: YAML SYNTAX ERROR in config.yaml → silent fallback to .env, all settings lost** — If `~/.hermes/config.yaml` has ANY YAML syntax error (bad indentation, unescaped characters in multiline blocks), gateway logs `Failed to process config.yaml — falling back to .env / gateway.json values` and IGNORES all config.yaml settings including `telegram.proxy_url`. **Symptom:** Gateway starts, adapter connects directly, `TELEGRAM_PROXY` is empty. **Diagnosis:** `grep "Failed to process config.yaml" ~/.hermes/logs/gateway.log`. **Fix:** Validate: `python -c "import yaml; yaml.safe_load(open('C:/Users/Asus/.hermes/config.yaml')); print('OK')"`.
- **CRITICAL: `config.yaml` missing `telegram:` section → proxy silently ignored** — The gateway's `_apply_yaml_config` hook (adapter.py line 7392) sets `TELEGRAM_PROXY` env var from `telegram.proxy_url` in config.yaml. BUT if the `telegram:` key is entirely absent from config.yaml, `apply_yaml_config_fn` is skipped (gateway/config.py line 1062: `if not isinstance(platform_cfg, dict): continue`). The adapter then falls back to direct connection (no proxy) → `ConnectError` in countries where Telegram is blocked. **Symptom:** Gateway starts, shows "Telegram" adapter enabled, but `RemoteProtocolError` / `ConnectError` immediately. **Fix:** Add `telegram:` section to config.yaml with `proxy_url: "socks5://..."`. **Verification:** `hermes config set telegram.proxy_url "socks5://127.0.0.1:10806"` then restart gateway.
- **CRITICAL: SOCKS5 proxy + default `connect_timeout=5.0` → TLS handshake timeout** — httpx's TLS handshake through SOCKS5 proxy takes longer than direct connection. Default `connect_timeout=5.0` (adapter.py line 2206) is too short → `httpcore.ConnectTimeout` on `stream.start_tls()`. **Symptom:** `httpx.ConnectError` with traceback showing `httpcore._async.socks_proxy.py:266: stream = await stream.start_tls(**kwargs)`. **Fix:** Set `HERMES_TELEGRAM_HTTP_CONNECT_TIMEOUT=30` in env, or patch the adapter default from 5.0 to 30.0. **Verification:** Test with `python -c "import httpx, asyncio; asyncio.run((lambda: httpx.AsyncClient(proxy='socks5://127.0.0.1:10806', timeout=httpx.Timeout(connect=15)).get('https://api.telegram.org')))"` — should return 302, not timeout.
- **Gateway `__pycache__` blocks code changes** — After editing plugin files (e.g. `plugins/platforms/telegram/adapter.py`), Python loads cached `.pyc` from `__pycache__/` directories. Clear with `rm -rf plugins/platforms/telegram/__pycache__/` before restarting gateway. Without this, code patches appear to have no effect.
- **Zombie gateway processes on Windows** — Multiple gateway processes can accumulate (from `gateway stop` + `gateway run`, or background process restarts). `gateway list` may show "not running" even when processes exist (different HERMES_HOME or pid file cleanup race). **Diagnosis:** `wmic process where "Name='python.exe'" get ProcessId,CommandLine | grep gateway` then `netstat -ano | grep <PID> | grep 10806` to find which is actually connected to proxy. **Fix:** Kill all gateway python processes, clear `gateway.pid`/`gateway.lock`/`gateway_state.json`, then start fresh.
