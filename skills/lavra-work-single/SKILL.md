...
3. **Log Knowledge as You Work** *(required -- inline, not at the end)*
   ...
   **IMPORTANT: bd comments must be executed as shell commands, never left as code comments.**
   Putting `# bd comments add hermes-0kh.2 "LEARNED: ..."` inside a .py or .md file does nothing --
   those comments are dead text. You or your subagent must actually run:
   ```bash
   bd comments add {BEAD_ID} "LEARNED: {key technical insight}"
   ```
   If you delegate implementation to a subagent, instruct it explicitly to run bd comments
   after each completed task. Verify afterwards that comments actually exist in the bead.
...
### Delegation pitfall: manual vs subagent coding

When the user has a strong preference against manual work (profile says "delegate everything"),
do NOT write code yourself in Phase 2. Instead, delegate to subagents:

```python
result = delegate_task(
    context="project context, file paths, constraints",
    goal="Implement feature X in file Y...",
    toolsets=["terminal", "file"]
)
```

Instruct the subagent explicitly to:
- Run `bd comments add {BEAD_ID} "LEARNED: ..."` as shell commands (not code comments)
- Only modify files in scope (no refactoring adjacent code)
- Only `git add {specific files}` — never `git add -A` (causes timeouts in Hermes repo)

After delegation returns, verify:
- Syntax OK: `python -c "import ast; ast.parse(open('file.py').read()); print('Syntax OK')"`
- Knowledge comments exist: `bd show {BEAD_ID} | grep LEARNED`
- No debug artifacts or TODO stubs left in changed files

### `git add -A` pitfall: timeouts in large repos

Never run `git add -A` or `git add .` in a Hermes repo. The `lsp/`, `node_modules/`,
`chrome-debug-profile/`, and `cache/` directories cause 60s+ timeouts and truncated output.
Always stage specific files: `git add scripts/one_file.py`

### bd comments pitfall: dead notes in code files

If a subagent writes `# bd comments add BEAD_ID "LEARNED: ..."` as a Python/markdown comment,
the knowledge is NOT logged — those are dead notes. After delegation returns, check with
`bd show {BEAD_ID} | grep LEARNED`. If empty, run the bd commands yourself.

### Sequential epic cross-check

After closing the last bead in an epic, also close the epic itself:
```bash
bd close {EPIC_ID}
```
Epics are not auto-closed when all children are done.
