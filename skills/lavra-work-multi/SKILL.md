...
```bash
PROJECT_ROOT="${PROJECT_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || echo "$PWD")}"
bash "$PROJECT_ROOT/.opencode/hooks/extract-bead-context.sh" {BEAD_ID}
```
Store output as `{BEAD_CONTEXT}`. If `.opencode/hooks/extract-bead-context.sh` does not exist, fall back to `$PROJECT_ROOT/plugins/lavra/hooks/extract-bead-context.sh`.
...
```bash
AGENT_TEMPLATE=$(cat ".opencode/skills/lavra-work-multi/references/subagent-prompt.md")
```
