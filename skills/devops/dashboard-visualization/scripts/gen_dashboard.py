...
def parse_kanban_list(output):
    ...
    for line in output.split('\n'):
        ...
        if line.startswith('▶') or line.startswith('⊘') or line.startswith('✓'):
            # ▶ e3d20e1f  ready     (unassigned)          Salon bot: нужен второй токен от BotFather
            # ⊘ t_a70b1bbb  blocked   default               EverOS — запуск сервера
            # ✓ f8ed6f79  done      default               Fix broken cron jobs
            parts = line.split()
            if len(parts) >= 5:
                status = "ready" if line.startswith('▶') else "blocked" if line.startswith('⊘') else "done"
                task_id = parts[1]
                # parts[2] is status column (ready/blocked/done)
...
def parse_kanban_stats(output):
    ...
    for line in output.split('\n'):
        line = line.strip()
        if 'ready' in line.lower() and 'status' in line.lower():
            parts = line.split()
...
def parse_profiles(output):
    ...
    for line in output.split('\n'):
        line = line.strip()
        if line and not line.startswith('Profile') and not line.startswith('─') and not line.startswith('│ Profile'):
            parts = line.split()
            if len(parts) >= 4:
                name = parts[0]
