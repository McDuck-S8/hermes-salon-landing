...
metadata:
  hermes:
    ...
    related_skills: ["kanban-worker", "browser-automation"]
---
...
7. **Browser/web tasks.** For web scraping, form submission, booking, monitoring — create a task assigned to a profile with `browser-automation` skill loaded.
...
**Goal-queue sync:** When user has active goals in `goal_queue.json`, create kanban tasks for each active/open goal so work is visible on the board. Sync goal status → task status on completion.

## Pitfalls

**When user asks to open kanban / dashboard — stop everything and do it NOW.**
The kanban board is a live service that should already be running. When the user says "открой канбан" / "open kanban" / "покажи доску":
1. RUN `hermes kanban list` immediately to show the board
2. CHECK if dashboard is running: `hermes dashboard --status`
3. If not running: `hermes dashboard --port 9119` in background, present CLI board while it builds
4. Do NOT finish in-progress work first
5. Do NOT explain what you were doing
6. Do NOT say "search isn't working" or blame tools
User marks this as DEMAND-level priority. Delays cause significant frustration.
