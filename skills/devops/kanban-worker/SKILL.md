...
## Cross-session continuity (marginalia)

Tasks you create in one session will be encountered again in a future session. Your future self will NOT remember why they were created. **The description field alone is not enough** — it lives inside the Dolt database and isn't surfaced until you explicitly open the task.

**Rule:** Every time you create a task (via `kanban_create`, `bd create`, or CLI), immediately write a memory entry:

```python
# After successful kanban_create/bd create
import os
memory(
    action="add",
    target="memory",
    content=f"TASK:{task_id} — WHY: {short_reason} — DESC: {task_title} — STATUS: open",
)
```

**When you encounter an existing task:**
1. Read its description via `kanban_show` / `bd show`
2. `session_search(query=f'task {task_id}', limit=1)` to find the original context
3. Cross-reference with any `TASK:{task_id}` memory entries
4. Then decide — close, block, or work

**When you close a task:** write the outcome to memory so future sessions can trace the chain.

```python
# Before kanban_complete / bd close
memory(
    action="add",
    target="memory",
    content=f"TASK:{task_id} — OUTCOME: {summary}",
)
```

This is what the user calls «пометки на полях» — marginalia for your future self.
