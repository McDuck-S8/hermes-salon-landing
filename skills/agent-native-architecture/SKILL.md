...
<filesystem_first>
## Filesystem-First Agent Configuration (Hermes Implementation)

This session implemented a **filesystem-first architecture for Hermes agents** directly inspired by eve's approach. The agent's entire configuration lives in a directory structure that IS the configuration:

```
agent/
├── instructions.md          # System prompt (always-on)
├── tools/                   # Custom tool definitions (Python files)
├── skills/                  # Skill references (eval.yaml + markdown)
├── schedules/               # Cron jobs as YAML files
├── channels/                # Message channel configs (Telegram, etc.)
...
**1. Files as Configuration (Parity)**
- Every capability is a file. Add a skill → add a file. Add a schedule → add a YAML. Add a sub-agent → add a directory.
- No separate config database, no UI for configuration management. Git is the source of truth.

**2. Approval System (Product Implications)**
- Three levels: `always` (ask every time), `once` (ask once per session), `never` (default for safe tools)
- Risk tags (`financial`, `destructive`, `external`) map to policies
- Decorator-based: `@approval.always("Sends real money")`

**3. Skill Evaluation Framework (Testing)**
- Each skill can have `eval.yaml` with test cases
- Match types: `exact`, `contains`, `regex`, `json_schema`, `python`
- Runs via `python scripts/skill_eval.py skills/`

**4. Sub-Agent Delegation (Composability)**
- Sub-agents are declarative YAML + markdown
- Loaded on-demand via `delegate_task` with filtered toolset
- Each has its own instructions, tools, skills, model config

**5. Cron as Files (Schedules)**
- YAML files in `agent/schedules/` parsed to `cron/jobs.json`
- Supports both prompt (task mode) and handler forms

**Implementation Files Created:**
- `scripts/approval.py` — Approval levels, decorators, risk policies
- `scripts/skill_eval.py` — YAML-driven skill evaluation
- `scripts/subagent_loader.py` — Loads sub-agents from filesystem
- `scripts/cron_parser.py` — YAML → cron/jobs.json
- `agent/subagents/__init__.py` — Registry + delegate integration
</filesystem_first>
**Core Patterns:**
