...
from autonomy.feedback_store import FeedbackStore, ExecutionRecord, Outcome
...
def run_integration_test():
    ...
    # Add test data to all components
    ...
    fs.record_execution('pat_test', 'strategic', 'test_pattern', {}, Outcome.SUCCESS)
    ...
    # Save checkpoint
    pm.mark_dirty(ComponentName.TACTICAL_BUFFER)
    pm.mark_dirty(ComponentName.STRATEGIC_DB)
    pm.mark_dirty(ComponentName.FEEDBACK_STORE)
    pm.mark_dirty(ComponentName.DIRECTIVES)
    pm.mark_dirty(ComponentName.ONTOLOGY)
    pm.mark_dirty(ComponentName.ACTIVE_GOALS)
    pm.mark_dirty(ComponentName.DIALOG_HISTORY)
    ...
    for cp in pm.list_checkpoints():
        print(f'  {cp["checkpoint_id"]} | {cp["timestamp"]} | {len(cp["components"])} components')
