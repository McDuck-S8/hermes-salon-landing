#!/usr/bin/env bash
# validate-fix.sh — Детерминированная проверка результата работы.
# Запускается ЧЕКЕРОМ. Вывод: PASS или FAIL. Третьего не дано.
...
# 4. Check if scheme has revenue (at least 1 lead)
REVENUE=$(python -c "
import sys
sys.path.insert(0, '$HERMES_HOME/scripts')
from finance_core import FinanceCore
...
# 5. Check withdrawal path tested (at least 1 withdrawal_received)
WITHDRAWN=$(python -c "
import sys
sys.path.insert(0, '$HERMES_HOME/scripts')
from finance_core import FinanceCore
