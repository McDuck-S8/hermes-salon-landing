---
name: 2026-07-26-02-05-38
description: "Auto-generated from 2026-07-26_02-05-38.md"
trigger: "When user asks about 2026-07-26_02-05-38 concepts"
usage: 2026-07-26-02-05-38
Revisit: 2026-07-31
---
...
```
============================================================
FAST SENSOR RUN - 2026-07-25T23:00:34.384304+00:00
============================================================
...
# FAST Sensor Run Report — 2026-07-25T23:00:34Z

## Summary
- **Run type**: FAST (30-min interval)
- **Sensors executed**: Traffic Costs, Cached Offers Gap Calculation, Network Health, Finance Baselines
- **CRITICAL alerts**: 1 (score ≥ 70)
- **HIGH alerts**: 0 (score 50-69)
- **Telegram delivery**: Ready (formatted plain text, <8 lines, Exfil-Guard safe)
...
**Details:**
- **Offer**: Raid Shadow Legends (adm_002, admitad) — Gaming CPI, RU, $1.20 payout, CR 15% (mock), Approval 85%
- **Traffic**: RichAds RU gaming — CPC $4.50, CPM $80, Min Deposit $300
- **Gap**: ROI 240%, projected $1,080/day profit
- **Confidence**: 1.00 (CR=0.15 × Approval=0.85 × 10 = 1.275 → capped at 1.0)

⚠️ **MOCK DATA FLAG**: This is the only mock offer with CR=15% (unrealistic for push gaming CPI — realistic range 1-5%). Confidence 1.0 is an artifact of the mock CR value. **Requires live validation before deploy.**

---

## 📊 Sensor Outputs

### Traffic Costs (6 sources, all MOCK)
| Source | Geo | Vertical | CPC | CPM | Min Dep |
|--------|-----|----------|-----|-----|---------|
...
### Arbitrage Gaps (9 found, only 1 ≥ 50)
| Offer + Traffic | ROI | $/day | Conf | Score | Level |
|-----------------|-----|-------|------|-------|-------|
| adm_002 + richads gaming RU | 240% | 1080 | 1.00 | **70** | 🔴 CRITICAL |
| adm_001 + kadam finance RU | 244% | 2075 | 0.20 | 14 | LOG |
| adm_001 + facebook finance RU | 63% | 1125 | 0.20 | 14 | LOG |
| cit_002 + kadam finance RU | 82% | 697 | 0.24 | 12 | LOG |
| adm_001 + google finance RU | 33% | 725 | 0.20 | 10 | LOG |
| acb_001 + kadam nutra RU | 65% | 780 | 0.17 | 9 | LOG |
| cit_001 + kadam finance RU | 78% | 665 | 0.16 | 7 | LOG |
| cpt_002 + kadam nutra RU | 41% | 487 | 0.18 | 6 | LOG |
| acb_001 + tiktok nutra RU | 32% | 480 | 0.17 | 6 | LOG |

**Note**: 8/9 gaps have confidence 0.16-0.24 (finance/nutra mock CR 0.03-0.04 → low confidence). Only the gaming CPI mock (CR=0.15) produces high confidence.
...
1. **Mock data staleness**: Cached offers last refreshed 2026-07-03 (>22 days). Per skill spec: confidence should be halved for >7 days stale, halved again for >14 days. Current run does not apply staleness penalty.

2. **Unrealistic mock CR**: Gaming CPI CR=15% (mock) vs realistic 1-5% for push traffic. Produces artifact ROI 240% with Confidence 1.0.

3. **Creative Radar**: Not implemented (requires `browser-automation` + BrowserOS MCP).

4. **Root vs Skill script discrepancy**: Cron job `2e04bc051ee0` may run root script with old unbounded scoring. Verify cron calls skill script at `skills/autonomous-ai-agents/always-on-agent/scripts/fast_sensor_run.py`.

---

## Next Actions
1. **Validate Raid Shadow Legends + RichAds live** — Check actual CR on RichAds gaming RU before any test spend
2. **Refresh mock cache** — Update offer/traffic data or implement API integrations
3. **Apply staleness penalty** — Modify `fast_sensor_run.py` to halve confidence for data >7 days old
4. **Verify cron target** — Ensure cron job `2e04bc051ee0` executes the skill script, not root script

---

**State persisted**: Finance core baselines logged, alert emitted to Telegram queue (plain text format, Exfil-Guard safe).
