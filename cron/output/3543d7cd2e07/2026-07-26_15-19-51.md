---
name: 2026-07-26-15-19-51
description: "Auto-generated from 2026-07-26_15-19-51.md"
trigger: "When user asks about 2026-07-26_15-19-51 concepts"
usage: 2026-07-26-15-19-51
Revisit: 2026-07-31
---
...
I attempted to run the **always-on-agent MEDIUM sensor run** but encountered a **Windows path resolution bug** that prevents the sensor scripts from importing their dependencies.

### Root Cause
The `HERMES_HOME` environment variable is not set in the cron environment. When `Path("D:/Portable_Soft/hermes")` is resolved on Windows, `Path.resolve()` mangles the drive letter, turning `D:/` into `\d\` (without the colon). This breaks all imports from `skills/finance/arbitrage-sensors/scripts/` and `scripts/`.

This exact issue is documented in the **always-on-agent skill under "Pitfalls & Fixes → Windows Path Resolution in Sensor Scripts (2026-07-26)"**.

### Files Affected
- `scripts/medium_sensor_run.py` (the main MEDIUM sensor runner)
- `scripts/fast_sensor_run.py` (FAST sensor runner)
- All sensor scripts that use `Path(__file__).resolve().parents[n]` or `Path("D:/...")`

### Fix Required
Update the path resolution in sensor scripts to use **raw strings** for Windows paths:
...
Or use `os.path` functions instead of `pathlib.Path` for better Windows compatibility.

### What Was Accomplished
1. ✅ Identified the MEDIUM sensor script at `scripts/medium_sensor_run.py`
2. ✅ Verified all required sensor modules exist in `skills/finance/arbitrage-sensors/scripts/`:
   - `gap_calculator.py`
   - `cpa_scanner.py`
   - `format_short_alert.py`
   - `send_short_alert.py`
3. ✅ Verified `finance_core.py` exists in `scripts/`
4. ✅ Confirmed the MEDIUM sensor logic is complete: 8 networks (Admitad, CityAds, ActionPay, Ad1, **AdCombo, CPAlead, MaxBounty, CPATrend**), scoring (0-100), HIGH+ (score ≥50) Telegram alerts, deep-dive dispatch via `multi-agent-researcher`

### Next Steps
1. Apply the Windows path fix to `scripts/medium_sensor_run.py` and `scripts/fast_sensor_run.py`
2. Set `HERMES_HOME=D:\Portable_Soft\hermes` in the cron job environment
3. Re-run the MEDIUM sensor

The sensor logic itself is complete and ready - only the Windows path resolution needs fixing.
