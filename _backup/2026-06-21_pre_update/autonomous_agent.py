...
def evaluate_actions(state: dict, profile: dict) -> list[dict]:
    ...
    # ── 3e. Crystal Will tasks (bridge from crystal.py) ──
    crystal_tasks_file = CACHE_DIR / "crystal_tasks.json"
    if crystal_tasks_file.exists():
        try:
            with open(crystal_tasks_file, "r", encoding="utf-8") as f:
                crystal_tasks = json.load(f)
            for task in crystal_tasks:
                priority_map = {'high': 5, 'medium': 3, 'low': 1}
                prio = priority_map.get(task.get('priority', 'medium'), 3)
                candidates.append({
                    "id": f"crystal-{task['id']}",
                    "tier": TIER_PRODUCE,
                    "tier_name": "CRYSTAL",
                    "title": f"🧠 Воля: {task.get('title', '?')}",
                    "description": task.get('description', '') or task.get('title', ''),
                    "urgency": prio,
                    "impact": 8,
                    "execute_fn": _action_execute_crystal_task,
                    "_crystal_task": task,
                })
        except Exception as e:
            log(f"Error reading crystal tasks: {e}", "WARN")

    # === TIER 2: LEARN — Use tools and skills proactively ===

    # 2d. Run curiosity engine — discover new things
    candidates.append({
        "id": "learn-curiosity",
        "tier": TIER_LEARN,
        "tier_name": "LEARN",
        "title": "Scan for new tools and technologies",
        "description": "Run curiosity engine to find interesting things on GitHub, HN, Reddit",
        "urgency": 4,
        "impact": 6,
        "execute_fn": _action_curiosity_scan,
    })

    # 2e. Analyze knowledge graph for actionable insights
    candidates.append({
        "id": "learn-graph-analysis",
        "tier": TIER_LEARN,
        "tier_name": "LEARN",
        "title": "Analyze knowledge graph for patterns",
        "description": "Query graphify graph to find hidden connections and opportunities",
        "urgency": 3,
        "impact": 5,
        "execute_fn": _action_graph_analysis,
    })

    # 2f. Scan user needs and create proactive goals
    candidates.append({
        "id": "learn-user-needs",
        "tier": TIER_LEARN,
        "tier_name": "LEARN",
        "title": "Analyze user needs and create goals",
        "description": "Scan user_needs_profiler data and create goals for unmet needs",
        "urgency": 5,
        "impact": 7,
        "execute_fn": _action_user_needs_analysis,
    })

    # === TIER 3: PRODUCE — Generate value using available skills ===

    # 3f. Generate daily report from knowledge
    candidates.append({
        "id": "produce-daily-knowledge",
        "tier": TIER_PRODUCE,
        "tier_name": "PRODUCE",
        "title": "Generate knowledge report from graph",
        "description": "Analyze graph connections and produce actionable insights",
        "urgency": 2,
        "impact": 4,
        "execute_fn": _action_knowledge_report,
    })

    # 3g. Check and use available MCP tools
    candidates.append({
        "id": "produce-mcp-check",
        "tier": TIER_PRODUCE,
        "tier_name": "PRODUCE",
        "title": "Check MCP server status and capabilities",
        "description": "Verify context7, memory, and other MCP servers are accessible",
        "urgency": 2,
        "impact": 3,
        "execute_fn": _action_mcp_check,
    })
...
def compute_score(action: dict, state: dict) -> float:
    ...
    # Learning adjustment — weight from feedback_store
    try:
        from feedback_store import compute_weight
        weight = compute_weight(action["id"])
        score *= weight
        # Exploration bonus: less-used actions get a boost
        from feedback_store import get_action_history
        history = get_action_history(action["id"], last_n=20)
        times_used = len(history)
        if times_used < 5:
            exploration_bonus = 1.0 / (1 + 0.5 * times_used)
...
def pick_best_action(candidates: list[dict], state: dict) -> dict:
    ...
    Rotation logic: every 3rd run, force a non-goal action from LEARN or PRODUCE tier.
    This prevents goals from dominating and ensures the agent uses its tools.
    """
    for c in candidates:
        c["score"] = compute_score(c, state)

    # Count recent decisions to determine rotation
    recent = load_decisions()[-3:]
    goal_runs = sum(1 for d in recent if d.get("action_id", "").startswith("goal-"))

    # Every 3rd run dominated by goals, force a non-goal action
    if goal_runs >= 2:
        non_goal = [c for c in candidates if not c["id"].startswith("goal-") and c["tier"] in (TIER_LEARN, TIER_PRODUCE)]
        if non_goal:
            non_goal.sort(key=lambda a: a["score"], reverse=True)
            return non_goal[0]
...
def _action_curiosity_scan(state: dict, profile: dict) -> str:
    """Run curiosity engine to discover new things."""
    try:
        import subprocess
        result = subprocess.run(
            [sys.executable, str(HERMES_HOME / "scripts" / "curiosity_engine.py"), "--quick"],
            capture_output=True, text=True, timeout=60,
        )
        output = result.stdout.strip()
        # Count discoveries
        import re
        match = re.search(r"Found (\d+) new discoveries", output)
        count = int(match.group(1)) if match else 0
        return f"Curiosity scan: {count} new discoveries found"
    except Exception as e:
        return f"Curiosity scan failed: {e}"


def _action_graph_analysis(state: dict, profile: dict) -> str:
    """Analyze knowledge graph for patterns."""
    try:
        import json
        graph_file = HERMES_HOME / "graphify-out" / "graph.json"
        ...
        for link in data.get("links", []):
            connections[link.get("source", "")] += 1
            connections[link.get("target", "")] += 1
        top5 = connections.most_common(5)
        top_str = ", ".join("%s(%d)" % (n, c) for n, c in top5)
        return f"Graph: {nodes} nodes, {edges} edges. Top hubs: {top_str}"
    except Exception as e:
        return f"Graph analysis failed: {e}"


def _action_user_needs_analysis(state: dict, profile: dict) -> str:
    """Analyze user needs and create goals."""
    try:
        from user_needs_profiler import load_profile
        from goal_queue import create_goal, get_active_goals
        p = load_profile()
        needs = p.get("needs", [])
        existing = [g["title"].lower() for g in get_active_goals()]
        ...
        for n in needs[:5]:
            ...
            if not text or len(text) < 10:
                continue
            title = "Address need: %s" % text
            if any(text[:20].lower() in t for t in existing):
                continue
            priority = min(6, max(2, n.get("count", 1) // 5))
            create_goal(title, 2, priority, [], "Auto from user needs")
            created += 1

        return f"User needs: {len(needs)} tracked, {created} new goals created"
    except Exception as e:
        return f"User needs analysis failed: {e}"


def _action_knowledge_report(state: dict, profile: dict) -> str:
    """Generate knowledge report from graph."""
    try:
        import json
        from collections import Counter
        graph_file = HERMES_HOME / "graphify-out" / "graph.json"
        ...
        for n in nodes:
            ...
            fname = Path(src).stem if src else "unknown"
            sources[fname] += 1

        top_files = sources.most_common(5)
        report = "Top files by node count: "
        report += ", ".join("%s(%d)" % (f, c) for f, c in top_files)
        return report
    except Exception as e:
        return f"Knowledge report failed: {e}"


def _action_mcp_check(state: dict, profile: dict) -> str:
    """Check MCP server status."""
    try:
        # Check if memory graph exists
        memory_file = HERMES_HOME / "cache" / "mcp_memory.jsonl"
        if memory_file.exists():
            import json
            lines = memory_file.read_text(encoding="utf-8").strip().split("\n")
            entities = sum(1 for l in lines if l.strip())
            return f"MCP memory: {entities} entities in graph"
        return "MCP memory: no data yet"
    except Exception as e:
        return f"MCP check failed: {e}"
...
def _action_apply_suggestions(state: dict, profile: dict) -> str:
    ...
    applied_actions = []
    applied_count = 0
    seen_actions = set()  # Deduplicate actions
    max_actions = 30  # Hard limit to prevent runaway processing

    for suggestion in suggestions[:20]:  # Max 20 suggestions
        ...
        for action_text in recommended[:3]:  # Max 3 actions per suggestion
            action_key = f"{issue_type}:{action_text}"
            if action_key in seen_actions:
                continue
            seen_actions.add(action_key)
            if applied_count >= max_actions:
                break
...
# ═══════════════════════════════════════════════
# Bridge: execute tasks from crystal will
# ═══════════════════════════════════════════════

def _action_execute_crystal_task(state: dict, profile: dict) -> str:
    """Execute a task posted by crystal will (crystal.py).
    Reads first pending task from crystal_tasks.json, runs the script,
    and removes it from the queue."""
    from pathlib import Path
    
    crystal_tasks_file = CACHE_DIR / "crystal_tasks.json"
    if not crystal_tasks_file.exists():
        return "No crystal tasks file found"

    tasks_text = crystal_tasks_file.read_text(encoding='utf-8')
    if not tasks_text.strip() or tasks_text.strip() == '[]':
        return "Crystal task queue is empty"

    try:
        tasks = json.loads(tasks_text)
    except Exception as e:
        return f"Failed to parse crystal_tasks.json: {e}"
    ...
    script_name = task.get("title", "")

    if not script_name:
        return f"Crystal task '{task_id}' has no script name"
    ...
    if not script_path.exists():
        ...
        if not script_path.exists():
            return f"Crystal task '{task_id}': script '{script_name}' not found"

    log(f"Crystal will task: executing {script_name} ({task_id})", "INFO")
    ...
    try:
        ...
        if result.returncode == 0:
            out_preview = result.stdout.strip()[:200] if result.stdout.strip() else "(silent)"
            report = f"🧠 Воля → выполнен {script_name} (exit=0). {out_preview}"
        else:
            err_preview = result.stderr.strip()[:200] if result.stderr.strip() else "(no stderr)"
            report = f"🧠 Воля → {script_name} завершился с кодом {result.returncode}: {err_preview}"

        log(f"Crystal task '{task_id}' — {script_name}: exit={result.returncode}", "INFO")
    except subprocess.TimeoutExpired:
        report = f"🧠 Воля → {script_name} превысил таймаут (120с)"
        log(f"Crystal task '{task_id}' — {script_name}: timeout", "WARN")
    except Exception as e:
        report = f"🧠 Воля → ошибка запуска {script_name}: {e}"
        log(f"Crystal task '{task_id}' — {script_name}: error {e}", "WARN")

    # Remove the executed task from queue
    try:
        ...
        log(f"Failed to clean crystal task '{task_id}': {e}", "WARN")
...
def main():
    ...
    log(f"  Cron: {state['cron_health'].get('jobs_total', '?')} jobs, "
        f"{state['cron_health'].get('jobs_with_errors', '?')} with errors")
    ...
    # 5. Execute
    ACTION_LOG = HERMES_HOME / "cache" / "action_log.jsonl"
    if dry_run:
        ...
        try:
            ...
            # Inject _goal into state for goal-based actions
            if "_goal" in best:
                state["_goal"] = best["_goal"]

            # Execute with timeout (30 seconds max per action)
            import threading
            ...
            def run_action():
                try:
                    result_holder[0] = best["execute_fn"](state, profile)
                except Exception as e:
                    error_holder[0] = e
            ...
            try:
                ...
                action_entry = {
                    ...
                    "title": best.get("title", "?")[:100],
                    "score": best.get("score", 0),
                    "result": result[:500],
                    "status": "success",
                }
                with open(ACTION_LOG, "a", encoding="utf-8") as f:
                    f.write(_json.dumps(action_entry, ensure_ascii=False) + "\n")
            except Exception:
                pass
    ...
    # OUTPUT VALIDATION — check before showing to user
    try:
        from output_validator import validate_task, should_block, get_escalation_message
        # Build validation task from the action
        validation_task = {
            "type": "research" if "research" in best["id"].lower() or "scan" in best["id"].lower() else "general",
            "criteria": ["Все критерии задачи выполнены"],
            "max_budget": 100,
        }
        validation_output = {"proofs": [], "verified_amounts": [], "risks": []}
        
        # For research-type actions, check if output contains real evidence
        if "research" in best["id"].lower() or "scan" in best["id"].lower() or "curiosity" in best["id"].lower():
            # Check if result has real data vs placeholder
            if "no " in result.lower() or "0 " in result or "not found" in result.lower():
                validation_output["proofs"] = []
                validation_output["verified_amounts"] = []
            else:
                # Assume result has some data
                validation_output["proofs"] = [{"source": "agent", "title": result[:100]}]
                validation_output["verified_amounts"] = [1]  # placeholder
        
        validation = validate_task(validation_task, validation_output)
        if should_block(validation):
            log(f"OUTPUT BLOCKED: {validation.summary()[:200]}")
            # Record as blocked outcome
            try:
                from feedback_store import record_outcome
                record_outcome(best["id"], -0.5, evidence="Output blocked by validator")
            except ImportError:
                pass
    except ImportError:
        pass

    # Record outcome for weight learning (WP-3: feedback loop)
    try:
        ...
        log(f"Outcome recorded: {best['id']} -> {'success' if success else 'failure'}")
    except ImportError:
        pass

    # Update goal progress (WP-5)
    try:
    ...
    try:
        ...
        print("\nAutonomous Agent Run Complete")
        print("Action: [%s] %s" % (best['tier_name'], best['title'].encode('ascii', 'replace').decode('ascii')))
        print("Score: %s" % best['score'])
        print("Result: %s" % result[:200].encode('ascii', 'replace').decode('ascii'))
