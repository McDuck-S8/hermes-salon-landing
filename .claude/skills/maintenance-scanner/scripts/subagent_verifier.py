...
class FileVerifier:
    ...
    def _check_no_placeholders(self, content: str) -> List[VerificationResult]:
        ...
        placeholders = [
            ...
            "[...]", "...", "your_", "insert_", "add_",
            "example.com", "placeholder", "changeme"
        ]
        
        found = []
        for p in placeholders:
            if p.lower() in content.lower():
                # Allow in comments for TODO/FIXME
                lines = content.split('\n')
                for i, line in enumerate(lines):
                    if p.lower() in line.lower() and not line.strip().startswith('#'):
                        found.append(f"{p} at line {i+1}: {line.strip()[:80]}")
        
        if found:
            results.append(VerificationResult(
                "no_placeholders", False, 
                f"Found {len(found)} placeholder(s): {'; '.join(found[:5])}", "high"
            ))
        else:
            results.append(VerificationResult(
                "no_placeholders", True, "No placeholder text found", "low"
            ))
